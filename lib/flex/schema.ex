defmodule Flex.Schema do
  ...
  def add_analyzers(deps, _opts), do: deps

  defmacro flex(name, type \\ nil, do: block) do
  ...
  def add_not_analyzed_to_mappings(mappings) do
    for {key, mapping} <- mappings, into: %{} do
      {key, mapping |> Map.merge(%{index: "not_analyzed"})}
    end
  end
  ...
  def flex_mappings(mappings, _version),
    do: %{
      _all: %{enabled: false},
      properties: mappings |> add_not_analyzed_to_mappings()
    }

  def parse_flex_mappings(fields) do
    fields
  ...
  def flex_mapping({field, meta}, [{option, analyzer} | opts]),
    do: flex_mapping({field, meta}, [{option, [analyzer]} | opts])
    ...
    field =
      ...
      |> deep_merge(%{
        fields: %{
          ...
          analyzer => %{type: field.type, analyzer: analyzer}
        }
      })
  ...
  def parse_schema(schema) do
    ...
    {mappings, meta} = parse_flex_mappings(schema)
